"""Implementation of a spatial-transformer network."""

import tensorflow as tf


# Edit by licensee: Changed name `spatial_transformer_network` to `spatial_transformer`.
def spatial_transformer(input_fmap, theta, out_dims=None, **kwargs):
    """
    ...
    # C = tf.shape(input_fmap)[3]  # Edit by licensee: Unused variable.
...
def bilinear_sampler(img, x, y):
    ...
    # B = tf.shape(img)[0]  # Edit by licensee: Unused variable.
    H = tf.shape(img)[1]
    W = tf.shape(img)[2]
    # C = tf.shape(img)[3]  # Edit by licensee: Unused variable.
